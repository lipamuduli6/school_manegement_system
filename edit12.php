...
if (isset($_POST["submit"])) {
  ...
  $sql = "UPDATE `studaa` SET `first_name`=?, `last_name`=?, `email`=?, `gender`=? WHERE id = ?";
  ...
  if ($result) {
    header("Location: index12.php?msg=Data updated successfully");
  } else {
...
<nav class="navbar navbar-light justify-content-center fs-3 mb-5"
    ...
    <div class="container">
        ...
        <?php
        $sql = "SELECT * FROM `studaa` WHERE id = ? LIMIT 1";
        $stmt = mysqli_prepare($conn, $sql);
